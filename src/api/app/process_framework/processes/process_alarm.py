...
def build_process_alarm_process() -> KernelProcess:
    ...
    # fan in step needs to wait for
    determine_affected_systems_step.on_event(
        DetermineAffectedSystemsStep.OutputEvents.CountOfAffectedSystems
    ).send_event_to(
        target=final_recommendation_step,
        function_name=FinalRecommendationStep.Functions.SetCountOfAffectedSystems,
    ...
    determine_affected_systems_step.on_event(
        ...
        target=run_analysis_step,
        function_name=RunAnalysisStep.Functions.RunAnalysis,
        parameter_name="params"
    )
    ...
    retrieve_alarm_documentation_step.on_event(
        ...
        target=final_recommendation_step,
        function_name=FinalRecommendationStep.Functions.RetrieveFinalRecommendation,
        parameter_name="params"
    )

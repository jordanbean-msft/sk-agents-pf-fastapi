...
async def on_event(partition_context, event):
    try:
        ...
        asyncio.create_task(run_alarm_process(decoded_event))
...
def start_alarm_event_listener():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to process events from Event Hub.") from e


@router.websocket("/alarm/{client_id}")
async def websocket_connect(websocket: WebSocket,
                            client_id: str,
                            websocket_connection_manager: ConnectionManagerClientDependency):
    """
